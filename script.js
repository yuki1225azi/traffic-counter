...
/* =========================================
   スクロール連動：ミニプレーヤー化 ＆ ドラッグ移動パッチ (センチネル方式・修正版)
   - 監視専用の要素(sentinel)を使って、確実な判定を行う
   ========================================= */
(function floatingPlayerPatch(){
  ...
  // 2. 監視用センチネル（透明な1pxの線）
  // これが画面上端から消えたら「スクロールした」とみなす
  const sentinel = document.createElement("div");
  sentinel.id = "video-sentinel";
  sentinel.style.height = "1px";
  sentinel.style.width = "100%";
  sentinel.style.pointerEvents = "none"; // 操作の邪魔をしない
  sentinel.style.marginTop = "-1px";     // 厳密な位置調整
  // 挿入場所: プレースホルダーよりもさらに前（一番上）
  container.parentNode.insertBefore(sentinel, placeholder);

  // 3. スクロール監視 (IntersectionObserver)
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      // プレースホルダーの高さを現在の映像エリアに合わせる（ガク付き防止）
      if(container.offsetHeight > 0 && !container.classList.contains("is-floating")){
         placeholder.style.height = container.offsetHeight + "px";
      }

      // 「監視線が画面外（上）に行った」 かつ 「見えていない」 場合
    ...
    threshold: 0, 
    rootMargin: "0px" // 余裕を持たせず厳密に判定
  });

  // 監視開始（映像ではなく、センチネルを監視する）
