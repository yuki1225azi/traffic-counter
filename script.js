...
let lastSnapAt=0;
// ★★★ 修正箇所 ★★★
// ログが1秒間隔で記録されるように、条件判定の変数を修正
function pushHourlySnapshotIfNeeded(){
    const t=Date.now();
    if(t-lastSnapAt<1e3) return; // lastInferTime -> lastSnapAt に修正
    lastSnapAt=t;
    const o=formatTimestamp(new Date(t)),
    s={timestamp:o,...countsCurrentHour};
    recordsHourly.push(s),updateLogDisplay()
}
...
function updateHourTitle(){
    const t=(hourWindowStart||new Date).getHours().toString().padStart(2,"0");
    DOM.hourTitle.textContent=`${t}時台の交通量`;
}
...
async function exportCSV(data, geo) {
    ...
    const header = '集計日時,乗用車,バス,トラック,バイク,自転車,歩行者\n';
    const rows = data.map(r => {
        return `"${r.timestamp}",${r.car},${r.bus},${r.truck},${r.motorcycle},${r.bicycle},${r.person}`;
    }).join('\n');
    const csv = `\uFEFF${metadata}\n${header}${rows}`;
